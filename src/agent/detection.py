...
from src.agent.constants import PREDEFINED_CLASS, PREDEFINED_CLASSES_BY_GROUP
...
class ObjectDectector:
    ...
    def __set_model(self, model_id, device, **kwargs):
        ...
        dn_model = AutoModelForZeroShotObjectDetection.from_pretrained(
            model_id).to(device)
    ...
    def invoke(self,
        ...
        """
        labels = list(PREDEFINED_CLASSES_BY_GROUP[context])
        device = "cuda" if torch.cuda.is_available() else "cpu"
