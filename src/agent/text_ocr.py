...
class OCRAgent:
    ...
    def __set_model(self):
        ...
        model = ocr = PaddleOCR(
            enable_mkldnn=True, use_angle_cls=True, lang='en')
        return model
    ...
    def invoke(self, image, keywords=keywords):
        ...
        if result and result[0]:
            ...
            mapped_text = self._map_to_closest_keyword(
    ...
    def _map_to_closest_keyword(self, text, keywords):
        ...
        for word in words:
            match, score = process.extractOne(
                word.lower(), keywords, scorer=fuzz.token_sort_ratio)
    ...
    def _count_keywords(self, text, keywords):
        keyword_counts = {keyword: text.lower().split().count(keyword)
